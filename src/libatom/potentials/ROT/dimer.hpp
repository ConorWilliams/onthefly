...
public:
  ...
  Dimer(Options const& opt, std::unique_ptr<Base> to_wrap) : m_opt(opt), m_core(opt.n), m_wrapped(std::move(to_wrap)) {}
   ...
   */
  floating rcut() const override {
    return m_wrapped->rcut() + m_opt.delta_r * 2;
  }
