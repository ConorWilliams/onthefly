    ...
    for (std::size_t z = 0; z < x.size(); ++z) {
      ...
      if (!x(Frozen{}, z)) {
        ...
        x.hess(m_aux(Hidx{}, z), m_aux(Hidx{}, z)).matrix()
            = m_data->f(x(AtomicNum{}, z)).fpp(m_aux(Rho{}, z)) * v * v.transpose();
        ...
        nl.for_neighbours(z, rcut(), [&](std::size_t ap, floating r, Vec3<floating> const &dr) {
          ...
          floating A = (m_data->v(x(AtomicNum{}, a), x(AtomicNum{}, z)).fp(r) +
                        ...
                        m_data->f(x(AtomicNum{}, z)).fp(m_aux(Rho{}, z))
                            * m_data->phi(x(AtomicNum{}, a), x(AtomicNum{}, z)).fp(r)
                        +

                        m_data->f(x(AtomicNum{}, a)).fp(m_aux(Rho{}, a))
                            * m_data->phi(x(AtomicNum{}, z), x(AtomicNum{}, a)).fp(r))
                       / r;
          ...
          floating B = m_data->v(x(AtomicNum{}, a), x(AtomicNum{}, z)).fpp(r) +
                       ...
                       m_data->f(x(AtomicNum{}, z)).fp(m_aux(Rho{}, z))
                           * m_data->phi(x(AtomicNum{}, a), x(AtomicNum{}, z)).fpp(r)
                       +

                       m_data->f(x(AtomicNum{}, a)).fp(m_aux(Rho{}, a))
                           * m_data->phi(x(AtomicNum{}, z), x(AtomicNum{}, a)).fpp(r);

          floating ABFpp
              = (A - B
                 - m_data->f(x(AtomicNum{}, a)).fpp(m_aux(Rho{}, a))
                       * ipow<2>(m_data->phi(x(AtomicNum{}, z), x(AtomicNum{}, a)).fp(r)))
                / (r * r);

          x.hess(m_aux(Hidx{}, z), m_aux(Hidx{}, z)).matrix()
              += A * Eigen::Matrix<floating, spatial_dims, spatial_dims>::Identity()
                 - ABFpp * dr.matrix() * dr.matrix().transpose();
          ...
          if (!x(Frozen{}, a)) {
            ...
            floating ur = m_data->f(x(AtomicNum{}, z)).fpp(m_aux(Rho{}, z))
                          * m_data->phi(x(AtomicNum{}, a), x(AtomicNum{}, z)).fp(r) / r;

            floating ru = m_data->f(x(AtomicNum{}, a)).fpp(m_aux(Rho{}, a))
                          * m_data->phi(x(AtomicNum{}, z), x(AtomicNum{}, a)).fp(r) / r;

            x.hess(m_aux(Hidx{}, z), m_aux(Hidx{}, a)).matrix()
                = -BArr * dr.matrix() * dr.matrix().transpose()
                  - A * Eigen::Matrix<floating, spatial_dims, spatial_dims>::Identity()
                  + ur * m_aux(Mu{}, z).matrix() * dr.matrix().transpose()
                  - ru * m_aux(Mu{}, a).matrix() * dr.matrix().transpose();
          }
        });
      }
    }
      ...
      nl.for_neighbours(a, rcut(), [&](std::size_t ep, floating r_e, auto const &dr_ae) {
        nl.for_neighbours(a, rcut(), [&](std::size_t gp, floating r_g, auto const &dr_ag) {
          ...
          if (e != g && !x(Frozen{}, e) && !x(Frozen{}, g)) {
            ...
            floating mag = ddFg / (r_e * r_g)
                           * m_data->phi(x(AtomicNum{}, g), x(AtomicNum{}, a)).fp(r_g)
                           * m_data->phi(x(AtomicNum{}, e), x(AtomicNum{}, a)).fp(r_e);

            x.hess(m_aux(Hidx{}, e), m_aux(Hidx{}, g)).matrix()
                += mag * dr_ae.matrix() * dr_ag.matrix().transpose();
          }
        });
      });
    }
