...
template <typename Data> class CellList {
public:
...
template <typename Data> void CellList<Data>::load(double rcut, Supercell const &cell) {
...
template <typename Data> void CellList<Data>::broadcast_ghost_data() {
...
void CellList<Data>::neigh_reduce(neigh_atom *atom, F &&f) {
...
// Maps canonical image position to integer
template <typename Data> int CellList<Data>::lambda(neigh_atom const &atom) const {
  int i = atom.vec[0] * _inv_cell[0];
  int j = atom.vec[1] * _inv_cell[1];
  int k = atom.vec[2] * _inv_cell[2];
...
template <typename Data> void CellList<Data>::load_simbox(double rcut, Simbox const &box) {
...
template <typename Data> void CellList<Data>::make_ghosts() {
  for (int i = 0; i < 3; ++i) {
