...
public:
   ...
   */
  Fingerprint const& fingerprint() const noexcept {
...
public:
   ...
   */
  EnvCell(Options const& opt, OrthoSimBox const& box)
      : m_nl(box, opt.r_env){};
   ...
   */
  LocalEnv const& operator[](std::size_t i) const noexcept {
    return m_envs[i];
  }
   ...
   * @brief Get the ith local environment
   */
  LocalEnv& operator[](std::size_t i) noexcept {
    return m_envs[i];
  }
