...
# times r in the tabulation, range 5.3 angstrom
...
# ( λ , k, C 0 , B 0 , r 0 , r 1 )=(1, 1.5, 0.23, 3.24, 0.9Å, 2.5Å)
...
def correct(r):
    if r < r_H_0:
        ...
        r_p = r - r_H_0
        return (
            C_H0 * pow(r_p / lam_H, k_H - 1) * math.exp(-pow(r_p / lam_H, k_H))
        )
    else:
        r_p = r - r_H_0
        return (
            C_H0
            * pow(r_p / lam_H, k_H - 1)
            * math.exp(-pow(r_p / lam_H, k_H) - B_H0 * pow(r - r_H_1, 2))
        )


def phi_H_H(r):  # new {as good as rho_H_H}
    if r <= r_cut_phi_HH:
        return r * (s(r) * (E_mol(r) - 2 * F_H_rho(rho_H_H_r(r))) + correct(r))
    else:
        return r * correct(r)
